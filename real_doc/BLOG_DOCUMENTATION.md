...
**Image must be sent as a file** in `multipart/form-data`; the API does not update the stored image from a URL string on update.
...
**Troubleshooting: Blog image not updating in the database**

1. **Queue worker not running** — The backend updates the blog image in a queued job (`StoreBlogImage`). If no worker is running, the job never runs and the database is never updated. **Fix:** On the backend server, run `php artisan queue:work` (or your configured queue worker). After saving, the new image will appear once the job runs (often within a few seconds).
2. **Sending URL instead of file** — To change the image on update, the client must send the new image as a **file** in `multipart/form-data` with the key `image`. Sending only a URL in JSON (e.g. `image_url`) does not update the stored image; the API expects a file for image replacement.
